...
class PersonMaskUltraV2:
    ...
    def person_mask_ultra_v2(self, images, face, hair, body, clothes,
        ...
        if self.model_buffer is None:
            log(f"{NODE_NAME} 初始化模型...", message_type='info')
            ...
            # 创建分割器选项
            image_segmenter_base_options = mp.tasks.BaseOptions(model_asset_buffer=self.model_buffer)
        ...
        for image in images:
            ...
            segmented_masks = self.segmenter.segment(media_pipe_image)
            masks = []
            if background:
